...
class ModelConfig:
    ...
    mask_id: Optional[Union[int, float]] = None
    ...
    # Mamba configs
    dt_rank: Optional[Union[int, str]] = None  # rank of the diffusion tensor
    d_state: Optional[int] = None  # N in paper/comments
    expand_factor: Optional[int] = None  # E in paper/comments
    d_conv: Optional[int] = None
    dt_min: Optional[float] = None
    dt_max: Optional[float] = None
    dt_init: Optional[Literal["random", "constant"]] = None
    dt_scale: Optional[float] = None
    dt_init_floor: Optional[float] = None
    rms_norm_eps: Optional[float] = None
    conv_bias: Optional[bool] = None
    inner_layernorms: Optional[bool] = None  # apply layernorms to internal activations
    pscan: Optional[bool] = None  # use parallel scan mode or sequential mode when training
    use_cuda: Optional[bool] = None  # use official CUDA implementation when training
...
class Model(nn.Module):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            module,
            (nn.LayerNorm,) + ((nn.RMSNorm,) if getattr(nn, "RMSNorm", None) is not None else ()),
        ):
    ...
    def enable_attention_collection(self):
        ...
        if self.config.model_type == "gpt":
            ...
            raise NotImplementedError(f"Attention collection not implemented for {self.config.model_type}")
    ...
    def get_attention_maps(self, layer_idx=None):
        ...
        if layer_idx is not None:
            return self.blocks[layer_idx].attn.attention_maps
        else:
            return [block.attn.attention_maps for block in self.blocks]
